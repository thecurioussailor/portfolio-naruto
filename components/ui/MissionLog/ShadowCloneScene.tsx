...
function measureSeat(
  ...
  if (cfg.zone === "above") {
    ...
    const cx = H.left - L.left + H.width * ax;
    const bottom = H.top - L.top - ay + cfg.height;
    const y = bottom - cfg.height;
  ...
  if (cfg.zone === "flank") {
    ...
    const fx = cfg.fx ?? 0;
    const w = cfg.height * aspect;
    const bottom = H.top - L.top + H.height * fy + cfg.height;
    const y = bottom - cfg.height;
    const cx =
      cfg.side === "left"
        ? H.left - L.left - fx - w / 2
        : H.right - L.left + fx + w / 2;
    const x = cfg.side === "left"
      ? H.left - L.left - fx - w
      : H.right - L.left + fx;
    return { x, y, cx, bottom };
  }

  // letter zone (default)
  const letters = heading.querySelectorAll<HTMLElement>("[data-letter]");
  const idx = cfg.letterIndex ?? 0;
  const r = letters[idx].getBoundingClientRect();
  const w = cfg.height * aspect;
  const cx = r.left - L.left + r.width / 2 + (cfg.dx ?? 0) * r.width;
  const bottom = r.top - L.top + r.height * (cfg.glyphTop ?? 0) + (cfg.overlap ?? 0);
...
export function ShadowCloneScene({ cardsRef }: ShadowCloneSceneProps) {
  ...
  // Set of clone indices currently visible
  const [visibleClones, setVisibleClones] = useState<Set<number>>(new Set());
  ...
  useEffect(() => {
    if (phase === "clones") {
      seats.forEach((_, i) => {
        setTimeout(() => setVisibleClones((prev) => new Set(prev).add(i)), i * TIMELINE.poofStagger);
      });
    }
    ...
    if (phase === "departing") {
      ...
      order.forEach((seatIdx, i) => {
        setTimeout(() => {
          // Poof fires, then clone disappears shortly after
          puff(seats[seatIdx].cx, seats[seatIdx].bottom - CLONES[seatIdx].height / 2);
          setTimeout(() => {
            setVisibleClones((prev) => {
  ...
  const origin = seats.length
    ? {
        x: seats[0].x - 60,
        y: Math.min(...seats.map((s) => s.bottom)) + 50,
      }
    : null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <AnimatePresence>
          {CLONES.map((cfg, i) => {
            if (!seats[i] || !visibleClones.has(i)) return null;
            return (
