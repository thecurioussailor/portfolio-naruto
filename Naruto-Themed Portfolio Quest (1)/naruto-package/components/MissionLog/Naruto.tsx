...
interface NarutoProps {
  phase: ScenePhase;
  /** Where he stands (px, relative to the scene layer) */
  x: number;
  y: number;
  height?: number;
}
...
function spriteFor(phase: ScenePhase): string {
  switch (phase) {
    case 'landed': return NarutoSprites.jump.land; // brief crouch, then idle via key change
...
/** The original: runs in, performs the jutsu, thumbs-up, runs out. */
export function Naruto({ phase, x, y, height = 62 }: NarutoProps) {
  ...
  return (
    ...
    >
      {/* look-around / nod live on the inner img so they compose with the run/walk transform */}
      <motion.img
        src={src}
        alt=""
        aria-hidden
